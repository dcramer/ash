...
class FileMemoryStore:
    ...
    async def get_memories_about_person(
        ...
        """Get active memories about a specific person."""
    ...
    async def find_memories_by_subject(
        self,
        person_ids: set[str],
        exclude_owner_user_id: str | None = None,
        limit: int = 20,
    ) -> list[MemoryEntry]:
        """Find active memories where any of the given person_ids are subjects."""
        if not person_ids:
            return []
        ...
        for memory in memories:
            ...
            if not memory.subject_person_ids:
                continue
            if not set(memory.subject_person_ids) & person_ids:
                continue
    ...
    async def find_hearsay_by_subject(
        self,
        person_ids: set[str],
        source_username: str,
        owner_user_id: str | None = None,
        ...
        """Find hearsay memories where any of the given person_ids are subjects."""
        if not person_ids:
            return []
        ...
        for memory in memories:
            if not memory.subject_person_ids:
                continue
            is_subject = bool(set(memory.subject_person_ids) & person_ids)
            if not is_subject:
                continue
