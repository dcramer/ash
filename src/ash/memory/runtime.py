...
async def initialize_memory_runtime(
    *,
    ...
    try:
        ...
        verification_llm, verification_model_name = _resolve_verification_model_runtime(
            config=config
        )
        extractor = MemoryExtractor(
            ...
            verification_enabled=config.memory.extraction_verification_enabled,
            verification_llm=verification_llm,
            verification_model=verification_model_name,
        )
        log.debug(
            "Memory extractor initialized (model=%s verification_model=%s)",
...
def _resolve_verification_model_runtime(
    *,
    config: AshConfig,
) -> tuple[LLMProvider, str]:
    """Resolve provider + concrete model name for verification.
    ...
    """
    requested = config.memory.extraction_verification_model
    ...
    if ":" in requested:
        ...
        if provider not in {"anthropic", "openai", "openai-oauth"}:
            raise ValueError(
                "Invalid extraction_verification_model provider. "
                "Expected anthropic/openai/openai-oauth."
            )
        if not model_name:
            raise ValueError(
                "Invalid extraction_verification_model: missing model name."
            )
