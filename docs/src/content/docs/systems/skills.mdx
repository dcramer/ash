...
For local dogfooding in this repo, you can point to the reference bridge script:
...
This skill remains a normal skill surface and relies on `ash-sb capability`; it
does not add integration hooks directly.
...
Internally, discovery is handled by `src/ash/skills/registry.py` and source metadata tracks built-in, installed, user, and workspace origins.
