...
| `extraction_verification_enabled` | `true` | Run second LLM pass to verify/rewrite extracted facts |
| `extraction_verification_model` | `null` | Verification model selector. `null` uses `[models.default]`; accepts alias (`"mini"`) or provider model (`"openai:gpt-5.2"` / `"gpt-5.2"`). |
| `extraction_confidence_threshold` | `0.7` | Minimum confidence to store extracted facts |

### Recommended Setups

Use a cheaper extraction model and a stronger verification model:
