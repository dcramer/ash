...
extraction_grounding_enabled = true
extraction_grounding_model = null
```
...
| `extraction_context_messages` | `8` | Message window used when building extraction context |
| `extraction_grounding_enabled` | `true` | Run second LLM pass to ground/rewrite extracted facts |
| `extraction_grounding_model` | `null` | Grounding model selector. `null` uses `[models.default]`; accepts alias (`"mini"`) or provider model (`"openai:gpt-5.2"` / `"gpt-5.2"`). |
| `extraction_confidence_threshold` | `0.7` | Minimum confidence to store extracted facts |

### Recommended Setups

Use a cheaper extraction model and a stronger grounding model:
...
extraction_grounding_enabled = true
extraction_grounding_model = "default"
```

Use a provider-qualified grounding model directly:

```toml
[memory]
extraction_model = "mini"
extraction_grounding_enabled = true
extraction_grounding_model = "openai:gpt-5.2"
```

Use a plain model name (resolved against the default model's provider):

```toml
[memory]
extraction_grounding_enabled = true
extraction_grounding_model = "gpt-5.2"
```
