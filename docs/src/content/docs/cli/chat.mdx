---
title: ash chat
description: Interactive chat with your assistant
sidebar:
  order: 4
---

import OptionList from '../../../components/OptionList.astro';

Start an interactive conversation or send a single prompt.

## Usage

```bash
uv run ash chat [OPTIONS] [PROMPT]
```

## Arguments

<OptionList
  items={[
    {
      name: 'PROMPT',
      description: 'Optional single prompt to send.',
    },
  ]}
/>

## Options

<OptionList
  items={[
    {
      name: '--model, -m',
      defaultValue: 'default',
      description: 'Model alias to use.',
    },
    {
      name: '--config, -c',
      defaultValue: 'auto',
      description: 'Path to config file.',
    },
    {
      name: '--streaming/--no-streaming',
      defaultValue: '--streaming',
      description: 'Enable or disable streaming output.',
    },
  ]}
/>

## Interactive Mode

Start an interactive session:

```bash
uv run ash chat
```

Type messages and press Enter. Type `exit` or `quit` to end.

## Single Prompt

Send a single prompt and exit:

```bash
uv run ash chat "What's the weather like?"
```

## Model Selection

Use a different model:

```bash
uv run ash chat --model fast "Quick question"
```

Models are defined in your config under `[models.*]`.

## Streaming

Disable streaming for CI/CD or scripting:

```bash
uv run ash chat --no-streaming "Generate a report"
```

## Examples

Interactive session:

```bash
uv run ash chat
> Hello!
Hi! How can I help you today?
> exit
```

Quick question:

```bash
uv run ash chat "How do I list files in bash?"
```

Using a specific model:

```bash
uv run ash chat -m reasoning "Explain quantum computing"
```
